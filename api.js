...
export default async function handler(request,response){const url=new URL(request.url,'https://'+(request.headers.host||'localhost'));const route=url.pathname;
 if(route==='/api/auth')return json(response,200,{adminEnabled:Boolean(process.env.ADMIN_TOKEN),authorized:isAdmin(request)});
 ...
 if(route==='/api/collect'){const cron=request.method==='GET';if(cron?!cronAuthorized(request):!isAdmin(request))return json(response,401,{error:'Unauthorized'});try{return json(response,200,{ok:true,...await runCollection(cron?'railway-cron':'manual')});}catch(error){return json(response,500,{error:error.message});}}
